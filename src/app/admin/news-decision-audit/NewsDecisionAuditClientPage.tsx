...
type AuditRow = { id: string; publishedAt: string | null; source: string | null; guid: string; headline: string; classification: string; impact: string; assets: Array<{ asset?: string; bias?: string; score?: number }>; isNew: boolean; classificationCompleted: boolean; semanticDedupCompleted: boolean; coverageRepairCompleted: boolean; duplicateOf: string | null; eventDuplicateOf?: string | null; driverTheme?: string | null; causalThemeId?: string | null; macroEventKey?: string | null; geoState?: string | null; semanticDirection?: string | null; semanticStrength?: string | null; directAssetSignals?: Array<{ asset?: string; bias?: string; score?: number }>; transmittedAssetSignals?: Array<{ asset?: string; bias?: string; score?: number }>; signValidationStatus?: string; boardLocked: boolean; displayEligible: boolean; visibleDestinations: string[]; finalDecisionCode: string; finalDecisionReason: string; secondaryReasons: unknown[]; ingestId: string | null; classificationJobId: string | null; provider: string | null; model: string | null; historicalDetail: boolean };
...
export default function NewsDecisionAuditClientPage() {
    ...
    const copyAll = async () => {
        ...
        const lines = ["time\tsource\tGUID\theadline\tclassification\timpact\tassets/scores\tdriver theme\tcausal theme\tgeo state\tsign validation\tevent duplicate\tdisplay eligible\tfinal decision\texact reason", ...full.rows.map((row) => [fmtTime(row.publishedAt), row.source ?? "", row.guid, row.headline, row.classification, row.impact, row.assets.map((asset) => `${asset.asset ?? ""}:${asset.score ?? 0}`).join(","), row.driverTheme ?? "", row.causalThemeId ?? "", row.geoState ?? "", row.signValidationStatus ?? "", row.duplicateOf ?? "", row.displayEligible ? "YES" : "NO", row.finalDecisionCode, row.finalDecisionReason].map((value) => String(value).replaceAll("\t", " ")).join("\t"))];
    ...
    return <main className="mx-auto max-w-[1800px] space-y-5 p-4 text-foreground lg:p-8">
        ...
        <section className="overflow-hidden rounded-xl border border-stroke bg-darkGrey"><div className="overflow-x-auto"><table className="min-w-[1800px] w-full text-left text-sm"><thead className="bg-black/20 text-xs uppercase text-[rgb(var(--secondary))]"><tr>{["Time (Dubai)","Source / GUID","Headline","Class / impact","Assets / scores","Cause / geo / sign","Checkpoint","Event duplicate","Display","Final decision","Exact reason"].map((heading) => <th key={heading} className="px-3 py-3">{heading}</th>)}</tr></thead><tbody>{data?.rows.length ? data.rows.map((row) => <tr key={row.id} className="border-t border-stroke align-top"><td className="whitespace-nowrap px-3 py-3">{fmtTime(row.publishedAt)}</td><td className="max-w-[220px] break-all px-3 py-3"><div>{row.source ?? "—"}</div><code className="text-[10px] text-[rgb(var(--secondary))]">{row.guid}</code></td><td className="max-w-[360px] px-3 py-3 font-medium">{row.headline}</td><td className="px-3 py-3"><span className="rounded bg-blue-500/15 px-2 py-1 text-xs">{row.classification}</span><div className="mt-2">{row.impact}</div></td><td className="px-3 py-3">{row.assets.map((asset) => `${asset.asset ?? "?"} ${asset.score ?? 0}`).join(", ") || "—"}</td><td className="max-w-[260px] px-3 py-3 text-xs">{row.driverTheme ?? "—"}<br /><code>{row.causalThemeId ?? "—"}</code><br />geo: {row.geoState ?? "—"}<br />sign: {row.signValidationStatus ?? "—"}</td><td className="px-3 py-3 text-xs">classification: {row.classificationCompleted ? "complete" : "pending"}<br />semantic: {row.semanticDedupCompleted ? "complete" : "pending"}<br />coverage: {row.coverageRepairCompleted ? "complete" : "pending"}</td><td className="px-3 py-3">{row.duplicateOf ? <span className="text-amber-300">DUPLICATE<br /><code className="text-[10px]">{row.duplicateOf}</code></span> : "—"}</td><td className="px-3 py-3">{row.displayEligible ? <span className="text-green-400">YES<br /><small>{row.visibleDestinations.join(", ")}</small></span> : <span className="text-red-300">NO</span>}</td><td className="px-3 py-3"><span className="rounded-full border border-stroke px-2 py-1 text-xs">{label(row.finalDecisionCode)}</span></td><td className="max-w-[360px] px-3 py-3">{row.finalDecisionReason}{row.secondaryReasons.length ? <details className="mt-2 text-xs text-[rgb(var(--secondary))]"><summary>Processing trace</summary>{row.secondaryReasons.join(", ")}</details> : null}{row.historicalDetail ? <div className="mt-2 text-[10px] text-amber-300">Historical/reconstructed detail</div> : null}</td></tr>) : <tr><td colSpan={11} className="px-4 py-12 text-center text-[rgb(var(--secondary))]">No persisted new items for this Dubai business day/filter.</td></tr>}</tbody></table></div>{data?.pagination && data.pagination.totalPages > 1 ? <div className="flex items-center justify-end gap-3 p-4 text-sm"><span>{data.pagination.total.toLocaleString()} rows</span><button disabled={page <= 1} onClick={() => setPage((value) => value - 1)} className="rounded border border-stroke px-3 py-1.5 disabled:opacity-40">Previous</button><span>Page {page} / {data.pagination.totalPages}</span><button disabled={page >= data.pagination.totalPages} onClick={() => setPage((value) => value + 1)} className="rounded border border-stroke px-3 py-1.5 disabled:opacity-40">Next</button></div> : null}</section>
    </main>;
}
